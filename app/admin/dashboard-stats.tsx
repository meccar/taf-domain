...
export async function DashboardStats() {
  ...
  const stats = [
    { label: "Tổng số người dùng", value: userCount ?? 0, icon: Users },
    { label: "Tổng số bài viết", value: postCount ?? 0, icon: FileText },
    {
      label: "Liên hệ mới",
    ...
    { label: "Tổng số liên hệ", value: contactCount ?? 0, icon: Mail },
  ];
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {stats.map(({ label, value, icon: Icon, highlight }) => (
        <Card key={label}>
          ...
          <CardContent>
            <p
              className={`text-2xl font-bold ${highlight ? "text-orange-500" : ""}`}
            >
              {value}
            </p>
          </CardContent>
